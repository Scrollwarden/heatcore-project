...
class ChunkTerrain:
    ...
    def update_detail(self, new_level_of_detail) -> None:
        ...
        for x in range(vertices_per_line):
            for y in range(vertices_per_line):
                # Vertices
                if y % 2 == 0:
                    self.vertices[x, y] = (self.coord.x * CHUNK_SIZE + x, self.height_data[x, y], self.coord.y * CHUNK_SIZE + y)
                else:
                    self.vertices[x, y] = (self.coord.x * CHUNK_SIZE + x + 0.5, self.height_data[x, y], self.coord.y * CHUNK_SIZE + y)
